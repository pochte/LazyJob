...
function Targeting()
    while Start_Engine do
        ...
        if player and player.status ~= 1 then
            if is_resting then
                -- Rest owns the idle state. follow() alone starts
                -- movement, which cancels /heal client-side in FFXI --
                -- independent of the explicit /target call below,
                -- which only fires once already adjacent. Without
                -- this, autotarget was yanking the character out of
                -- every kneel within half a second while is_resting
                -- stayed stuck true, thinking it was still resting.
                windower.ffxi.follow(0)
