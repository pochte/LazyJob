...
-- NAME (e.g. "Fusion"), not the list of elements that actually
-- receive the magic burst damage bonus on it. That list lives
-- in sc_info, keyed by the same name -- e.g.
--
--     sc_info['Fusion'] = {'Fire', 'Light', ...}
--
-- meaning a Fusion chain gives a burst bonus to both Fire and
...
local function action_handler(act)
    ...
    if add_effect and skillchain_ids[add_effect.message_id] then
        ...
        local level=sc_data and sc_data.lvl or 1
...
ActionPacket.open_listener(function(act)
    ...
    if not ok then
        windower.add_to_chat(
            2,
            '[SC] action_handler error: '..tostring(err)
        )
    end
end)
