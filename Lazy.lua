...
local origin_z_tolerance = 15   -- yalms of vertical separation before we
                                 -- treat origin as unreachable rather than
                                 -- just "far" (e.g. fell off a ledge onto
                                 -- a lower shelf that's close in x/y but
                                 -- walled off)
...
-- Stuck-recovery state for Path_To_Origin (see below). Tracks whether
-- we're actually closing distance to origin over time, independent of
-- path_tick, which just cycles turn/run and says nothing about progress.
local path_last_distance      = nil
local path_last_progress_time = nil
local path_stuck_alerted      = false
local PATH_STUCK_TIMEOUT      = 8   -- seconds with no real progress before we give up
local PATH_STUCK_EPSILON      = 1   -- yalms of shrinkage that counts as "progress"

-- If we're cut off from origin by elevation (see origin_z_tolerance) for
-- this long, stop waiting on a home base we can't get back to and
-- re-anchor origin to wherever we actually are -- lets autotargeting pick
-- something reachable instead of staying idle indefinitely.
local origin_unreachable_since   = nil
local ORIGIN_UNREACHABLE_TIMEOUT = 600   -- 10 minutes
...
local ws_index           = 1
local PlayerH            = 0
local engaged_since      = nil   -- os.clock() timestamp; nil when not engaged
...
local self_ability_last_cast = {}   -- interval-based self job abilities
local haste_last_cast     = {}
local debuff_last_cast    = {}
local refresh_last_cast   = {}
local party_buff_last_cast = {}   -- new generic tracker
...
-- These must remain global – magicburst.lua reads them
current_job    = nil
active_profile = nil
...
function Update_Job_Profile()
    ...
    -- reset all timers on job change
    self_buff_last_cast  = {}
...
windower.register_event('incoming chunk', function(id, data)
    ...
    if action.Category == 4 then          -- spell finished
        isCasting = false
    ...
    elseif action.Category == 8 then      -- spell started
        isCasting = true
...
-- DEATH WATCH
--
-- Tracks whatever last actually landed a hit on us (any actor,
-- not just our own actions) so that if we die, //lazy can say
-- what killed us instead of just going quiet.
------------------------------------------------------------

local last_damage_source = nil   -- name of whatever last hit us
local last_damage_kind   = nil   -- resolved spell/ability/WS name, if any
...
------------------------------------------------------------
-- ORIGIN DISTANCE (elevation-aware)
--
-- Plain x/y distance treats "underneath a cliff" the same as
-- "next to it" -- which is exactly what sent us running into
-- a wall for hours after falling off a ledge. If the vertical
-- separation is large, treat origin/the point as unreachable
-- by straight-line running rather than just "a bit far".
------------------------------------------------------------
...
function Path_To_Origin()
    ...
    --------------------------------------------------------
    -- Different elevation entirely -- e.g. knocked off a
    -- ledge onto a lower shelf that's close in x/y but has
    -- a cliff face in between. Don't run straight at it.
    --
    -- If this drags on, stop waiting on an origin we can't
    -- get back to and re-anchor here instead, so targeting
    -- picks something reachable from where we actually are.
    --------------------------------------------------------
    ...
    if distance > 3 then
        ...
        -- STUCK DETECTION
        --
        -- path_tick just cycles turn/run and tells us
        -- nothing about whether we're actually getting
        -- closer. If distance hasn't meaningfully shrunk in
        -- PATH_STUCK_TIMEOUT seconds, we're wedged on
        -- terrain -- give up instead of grinding into a wall
        -- indefinitely.
        ----------------------------------------------------
        local now = os.clock()
...
-- FOLLOW MONITOR
--
-- windower.ffxi.follow(index) makes the client continuously
-- run toward that entity's current position -- it doesn't
-- stop on its own once you're close.
--
-- Universal rule, independent of job/profile:
--   * mid-cast (or mid-WS/ability/item) -> hold still
--   * not busy and outside melee range  -> close the gap
--   * not busy and already in range     -> hold position
--   * target moves back out of range    -> close the gap again
--
-- No job-based exceptions -- magic_burst doesn't mean "stay at
-- range": RDM bursts and melees, GEO wants melee range to keep
-- buffing the melee, NIN bursts with ninjutsu while meleeing.
-- Every job wants the same thing here: hold still while
-- casting/acting, otherwise stay within stabbing range.
------------------------------------------------------------
...
function Combat()
    ...
    -- Track continuous engaged duration, for stuck recovery below.
    ...
    -- STUCK RECOVERY
    --
    -- Engaged but wedged on terrain, never landing a hit --
    -- Provoke (re-faces the target as a side effect on its
    -- own, but we turn explicitly too) and keep retrying.
    -- Provoke's own recast naturally rate-limits actual
    -- casts; this just checks every tick once due.
    --------------------------------------------------------
    ...
    if active_profile.magic_burst then
        if Try_Magic_Burst() then return end
...
------------------------------------------------------------
-- BUFF SYSTEM (fully generic)
------------------------------------------------------------

function Buff_Tick()
    if not settings.buffs_active then return end

    -- pending cast timeout
    ...
    -- SELF ABILITIES
    --
    -- Same idea as SELF BUFFS above, but for job abilities
    -- (Haste Samba, etc.) instead of spells. Abilities don't
    -- go through the spell-finished confirmation packet, so
    -- this just records the cast time optimistically when
    -- sent -- the ability's own in-game recast is the real
    -- backstop against spamming it.
    --
    -- Merges the profile's own self_abilities with anything
    -- settings.lua's subjob_abilities grants for the current
    -- sub_job (e.g. Haste Samba for anyone subbing DNC) -- so
    -- this fires regardless of which job profile is active.
    --------------------------------------------------------
    ...
    do
        ...
        if debug_self_abilities then
            windower.add_to_chat(8, '[Lazy debug] sub_job=' .. tostring(sub)
                .. ' haste_samba_active=' .. tostring(haste_samba_active)
                .. ' has_table=' .. tostring(subjob_abilities and subjob_abilities[sub] ~= nil))
        end
    ...
    for _, ab in ipairs(self_ability_list) do
        ...
        if debug_self_abilities then
            local ability = res.job_abilities:with('name', ab.name)
            local recasts = windower.ffxi.get_ability_recasts()
            windower.add_to_chat(8, '[Lazy debug] ' .. ab.name
                .. ' due=' .. tostring(due)
                .. ' known_ability=' .. tostring(ability ~= nil)
                .. ' recast=' .. tostring(ability and recasts[ability.recast_id])
                .. ' can_cast=' .. tostring(Can_Cast_Ability(ab.name)))
        end
        if due then
    ...
    --------------------------------------------------------
    -- NEW GENERIC PARTY BUFFS (preferred)
    --------------------------------------------------------
    ...
    --------------------------------------------------------
    -- LEGACY HASTE SUPPORT (for older profiles)
    --
    -- Skipped entirely if this profile has a party_buffs
    -- table (the new system already covers haste there).
    -- Without this guard, a profile defining both would get
    -- Haste II tracked and cast through two independent
    -- timers/caches -- i.e. hasted twice, same bug as the
    -- old self-haste-twice issue but party-wide.
    --------------------------------------------------------
    ...
    --
    -- Same guard as above -- skipped if party_buffs exists,
    -- so a migrated profile can't double-cast Refresh III
    -- through both systems either.
    --------------------------------------------------------
...
-- TRUST RESUMMON
--
-- Snapshots which party slots are Trusts when Lazy starts,
-- then watches for any of them dropping out of the party
-- (dead) and resummons them. Player characters are never
-- touched -- only names captured at start-time that were
-- flagged as NPCs (Trusts occupy party slots the same way
-- players do; player pets like avatars/wyverns/automatons
-- don't show up in this list at all).
------------------------------------------------------------

local tracked_trusts      = {}   -- names of trusts present at //lazy start
local trust_resummon_last = {}   -- name -> os.clock() of last confirmed resummon
...
function Trust_Tick()
    ...
    -- Who's actually alive and in the party right now.
    local present = {}
    for _, key in ipairs({'p0','p1','p2','p3','p4','p5'}) do
        local m = party[key]
