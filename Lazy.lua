...
function Update_Movement_State()
    ...
    if last_known_pos then
        ...
        movement_state = (dx * dx + dy * dy) > (move_threshold * move_threshold)
    else
        movement_state = false
    end
...
local function Resolve_Attack_Name(param)
    ...
    local hit = res.monster_abilities[param]
        or res.spells[param]
...
windower.register_event('outgoing chunk', function(id, data)
    ...
    PlayerH = packets.parse('outgoing', data).Rotation
end)
...
windower.register_event('addon command', function(...)
    ...
    if command == 'start' then
        ...
        coroutine.schedule(MagicBurst_Monitor, 0)
...
function Set_Origin()
    ...
    origin_x, origin_y, origin_z = player.x, player.y, player.z
    path_last_distance = nil
    path_last_progress_time = nil
    path_stuck_alerted = false
    origin_unreachable_since = nil
...
function Movement_Locked()
    return isCasting or mp_resting
end
...
function Path_To_Origin()
    ...
    if distance == math.huge then
        ...
        local now = os.clock()
        origin_unreachable_since = origin_unreachable_since or now
        ...
        then
            ...
            if not path_stuck_alerted then
                windower.add_to_chat(
                    ...
                    PATH_STUCK_TIMEOUT .. 's) -- stopping autopath.'
                )
                path_stuck_alerted = true
            end
...
function Get_Party_Leader_ID()
    ...
    if party.party1_leader and party.party1_leader ~= 0 then
...
function Is_Party_Leader()
    ...
    local leader_id = Get_Party_Leader_ID()
    if not leader_id then return true end
...
function Can_Auto_Engage()
    return active_profile and active_profile.auto_engage == true
end
...
function Is_Valid_Combat_Target(mob)
    if not mob or not mob.valid_target or not mob.is_npc then return false end
...
function Is_Targetable_Monster(name)
    ...
    for _, monster in ipairs(targeting.monsters or {}) do
        if string.lower(name) == string.lower(monster) then return true end
    end
...
function Is_Targeting_Allowed(mob)
    if not Is_Valid_Combat_Target(mob) then return false end

    if settings.target and settings.target ~= '' then
        if string.lower(mob.name or '') ~= string.lower(settings.target) then
            return false
        end
    ...
    if targeting.only_alive and (not mob.hpp or mob.hpp <= 0) then return false end
    if targeting.only_unclaimed and mob.claim_id ~= 0 then return false end

    if targeting.within_origin and origin_x and mob.x
        and Origin_Distance(mob.x, mob.y, mob.z) > origin_radius
    then
        return false
    end
...
function Find_Named_Target(target_name)
    if not target_name or target_name == '' then return -1 end

    local candidates = {}

    for key, mob in pairs(windower.ffxi.get_mob_array() or {}) do
    ...
    for _, entry in ipairs(candidates) do
        ...
        if string.lower(mob.name or '') == string.lower(target_name)
...
function Find_Nearest_Target()
    ...
    for _, entry in ipairs(candidates) do
        if Is_Targeting_Allowed(entry.mob) then return entry.key end
    end
...
function Set_Target_By_Mob(mob)
    if not mob or not mob.id or not mob.index or mob.index <= 0 then
        return false
    end

    local current = windower.ffxi.get_mob_by_target('t')
    if current and current.id == mob.id then return true end
...
function Broadcast_Leader_Target()
    ...
    if target and target.id and target.index then
        target_id, target_index = target.id, target.index
    end
...
windower.register_event('ipc message', function(message)
    ...
    local id, index = message:match(
        '^lazy_target%s+(%d+)%s+(%d+)%s+(%d+)'
    )

    if not id or Is_Party_Leader() then return end
...
function Sync_To_Leader_Target()
    if Is_Party_Leader() or not leader_target_changed or isCasting then return end

    leader_target_changed = false

    if leader_target_id <= 0 or leader_target_index <= 0 then return end

    local target =
        windower.ffxi.get_mob_by_id(leader_target_id)
        or windower.ffxi.get_mob_by_index(leader_target_index)

    if target then Set_Target_By_Mob(target) end
end
...
function Leader_Acquire_Target()
    if not Is_Party_Leader() or not settings.autotarget then return nil end

    local current = windower.ffxi.get_mob_by_target('t')
    if Is_Targeting_Allowed(current) then return current end
    ...
    if not target_id or target_id <= 0 then return nil end

    local target = windower.ffxi.get_mob_by_index(target_id)
    if not target or not Is_Targeting_Allowed(target) then return nil end
    ...
    while Start_Engine do
        ...
        elseif Is_Party_Leader() then
            ...
            if not leader then
                ...
                local distance = leader.distance and math.sqrt(leader.distance)
...
function Engine()
    while Start_Engine do
        ...
        if player then
            ...
            if isBusy > 0 then
                -- Action_Delay is in seconds; Engine runs every 0.2s.
                isBusy = math.max(0, isBusy - 0.2)
            end
...
function Combat()
    ...
    if isCasting or mp_resting then return end

    local target = windower.ffxi.get_mob_by_target('t')
    local starter = Get_WS_Starter()

    if player.status == 1 then
        engaged_since = engaged_since or os.clock()
    else
        engaged_since = nil
    end
    ...
    then
        if Can_Cast_Ability('Provoke') then Cast_Ability('Provoke') end
        TurnToTarget()
    end
    ...
    -- MAGIC BURST
    --
    -- MagicBurst_Monitor owns magic for magic-burst profiles.
    -- Combat stays idle for those jobs so melee/WS logic cannot
    -- steal the action window.
    --------------------------------------------------------
    ...
    if not isCasting then TurnToTarget() end
...
function Cast_Ability(ability_name)
    if isCasting or isBusy > 0 then return false end

    local ability = res.job_abilities:with('name', ability_name)
    if not ability then return false end

    local recasts = windower.ffxi.get_ability_recasts()
    if recasts[ability.recast_id] ~= 0 then return false end
...
function Buff_Tick()
    ...
    for _, ability_name in ipairs(active_profile.job_abilities or {}) do
        if Can_Cast_Ability(ability_name) then
            if Cast_Ability(ability_name) then
                return
            end
        end
    end
    ...
    local mode = current_job == 'BLM' and BLM_Get_Mode and BLM_Get_Mode() or nil

    for _, buff in ipairs(active_profile.self_buffs or {}) do
        local allowed =
            (not buff.solo_only or mode == 'SOLO')
            and (not buff.party_only or mode == 'BURST')
            and (not buff.require_buff or buffactive[buff.require_buff])
        ...
        if allowed then
            ...
            if not last or now - last >= interval then
                for _, spell_name in ipairs(names) do
                    local spell = res.spells:with('name', spell_name)

                    if spell and Cast_Spell_On(spell_name, '<me>') then
                        pending_cast = {
    ...
    do
        ...
        if debug_self_abilities then
            windower.add_to_chat(
                ...
                ' has_table=' ..
                tostring(subjob_abilities and subjob_abilities[sub] ~= nil)
            )
        end

        if haste_samba_active and sub
